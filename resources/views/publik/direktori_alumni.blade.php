...
<div class="container container-card">
    <h2 class="text-center mb-5 section-title">Direktori Alumni</h2>
    <div class="row justify-content-center">
        @foreach($alumnis as $alumni)
        <div class="col-md-4 mb-4">
            <div class="alumni-card" onclick="showAlumniModal(this)"
                data-fullname="{{ $alumni->fullname }}"
                data-nim="{{ $alumni->nim }}"
                data-description="{{ $alumni->short_description }}"
                data-year="{{ $alumni->graduation_year }}"
                data-phone="{{ $alumni->active_phone_number }}"
                data-birth="{{ $alumni->date_of_birth ?? 'Tidak tersedia' }}"
                style="cursor: pointer;">
                <img src="https://ui-avatars.com/api/?name={{ urlencode($alumni->fullname) }}&background=010169&color=fff"
                    alt="Foto Alumni" class="alumni-photo">
                <h5 class="fw-bold mb-2">{{ $alumni->fullname }}</h5>
                <p class="text-muted">{{ $alumni->short_description }}</p>
            </div>
        </div>
        @endforeach
    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content text-dark">
            ...
            <div class="modal-body">
                ...
                <div id="modalDescription" class="mb-2"></div>
            </div>
        </div>
    </div>
</div>
...
<script>
    function showAlumniModal(el) {
        document.getElementById('modalFullname').textContent = el.dataset.fullname;
        document.getElementById('modalNim').textContent = el.dataset.nim;
        ...
        document.getElementById('modalBirth').textContent = el.dataset.birth;
